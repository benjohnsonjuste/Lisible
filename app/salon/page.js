...
export default function ForumPage() {
  ...
  // Injection directe du script Social Bar
  useEffect(() => {
    if (
      adVisible &&
      adRef.current &&
  ...
  async function loadMessages() {
    try {
      const res = await fetch(
        `https://api.github.com/repos/benjohnsonjuste/Lisible/contents/data/forum/messages`
      );
  ...
  const handlePostMessage = async () => {
    ...
    if (!user)
      return toast.error("Inscrivez-vous pour participer.");
    ...
    const userName =
      user.name || user.fullName || "Auteur";
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-20 bg-[#FCFBF9] min-h-screen">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-6">
        <div>
          <h1 className="text-7xl md:text-8xl font-black italic tracking-tighter text-slate-900 leading-none">
            Salon Lisible.
          </h1>
