...
import { 
  ...
  ShieldCheck, Loader2, Save, BookOpen, Star, TrendingUp, Sparkles, Key, Trash2, AlertTriangle, Mail, Landmark, Wallet
} from "lucide-react";
  ...
  const refreshUserData = async (email) => {
    try {
      ...
      if (res.ok) {
        ...
        setUser(freshUser);
        setFormData({ ...freshUser, penName: freshUser.penName || freshUser.name || "" });
      }
          ...
          <div className="bg-white rounded-[3rem] p-8 md:p-12 shadow-xl border border-slate-50 space-y-10">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-8 p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100">
               <div className="relative group">
                  <img src={formData.profilePic || `https://api.dicebear.com/7.x/avataaars/svg?seed=${user.email}`} className="w-32 h-32 rounded-[2.5rem] object-cover border-4 border-white shadow-2xl transition-all group-hover:scale-105" />
