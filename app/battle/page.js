...
import { 
  ...
  Heart, Share2, ArrowRight, RefreshCcw, Zap, Coins, Sparkles, AlignLeft, Gift, X
} from "lucide-react";
...
export default function BattlePoetique() {
  ...
  const sortBattleTexts = useCallback((data) => {
    return data
      .filter(i => i.isConcours === true || i.isConcours === "true" || i.genre === "Battle Poétique" || i.category === "Battle Poétique")
      .sort((a, b) => {
  ...
  const loadConcoursTexts = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setTexts(sortBattleTexts(rawList.filter(t => t !== null)));
    } catch (e) {
      toast.error("Connexion à l'Arène perdue.");
  ...
  const handleShare = async (e, item) => {
    ...
    try {
      if (navigator.share) await navigator.share({ title: `Votez pour "${item.title}" !`, url });
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-[#FCFBF9] gap-6 px-4">
      <div className="relative"><Loader2 className="animate-spin text-teal-600" size={50}/><Zap className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-teal-200" size={20} /></div>
      <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400">Calcul des rangs...</p>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#FCFBF9] font-sans pb-20 overflow-x-hidden">
          ...
          <div className="space-y-6">
            <div className="flex flex-wrap items-center gap-4">
              <div className="inline-flex items-center gap-2 bg-slate-900 text-white px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-2xl border border-slate-700">
                <Trophy size={14} className="text-amber-400 fill-amber-400" /> Battle Saison 2
              </div>
              {isRefreshing && <div className="flex items-center gap-2 text-teal-600 animate-pulse"><RefreshCcw size={12} className="animate-spin" /><span className="text-[9px] font-black uppercase">Live</span></div>}
            </div>
            <h1 className="text-4xl sm:text-6xl md:text-8xl font-black italic tracking-tighter text-slate-900 leading-[0.85]">L'Arène <br /><span className="text-teal-600">Sacrée.</span></h1>
            <p className="text-slate-400 font-medium max-w-md leading-relaxed text-sm">Seule la force du verbe compte dans cet affrontement.</p>
          </div>
        ...
        <main className="mt-12 md:mt-20">
          {texts.length > 0 ? (
            ...
            <div className="py-40 text-center space-y-10 bg-white rounded-[5rem] border-2 border-dashed border-slate-100">
              <Zap size={30} className="text-slate-200 mx-auto" />
              <h3 className="font-black uppercase text-slate-900 tracking-[0.3em]">L'arène est vide</h3>
              <Link href="/publier-battle" className="inline-flex items-center gap-3 bg-slate-900 text-white px-12 py-6 rounded-3xl font-black text-[11px] uppercase tracking-widest hover:bg-teal-600 transition-all">Lancer le défi <ArrowRight size={18} /></Link>
            </div>
          )}
        </main>
      </div>
      ...
      {giftRecipient && (
        <GiftModal
          ...
          contexte={{ type: "battle", refId: "battle", refTitre: "Battle Poétique" }}
          onClose={() => setGiftRecipient(null)}
        />
      )}

      <footer className="mt-32 text-center pb-10"><p className="text-[10px] font-black uppercase tracking-[0.5em] text-slate-300 flex items-center justify-center gap-4"><span className="w-8 h-px bg-slate-100"></span>Arène Officielle • {new Date().getFullYear()}<span className="w-8 h-px bg-slate-100"></span></p></footer>
    </div>
  );
}
