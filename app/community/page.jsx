        ...
        const statsMap = json.content.reduce((acc, pub) => {
          ...
          if (!acc[email]) {
            acc[email] = {
              ...
              email: email,
              image: pub.image || pub.authorImage || null,
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-20 bg-[#FCFBF9] min-h-screen">
      <header className="flex flex-col lg:flex-row justify-between mb-24 gap-8">
        ...
        <input type="text" placeholder="Rechercher..." className="w-full lg:w-96 bg-white border-2 border-slate-50 rounded-[2rem] px-8 py-5 shadow-xl outline-none focus:border-teal-500 transition-all" onChange={(e) => setSearchTerm(e.target.value)} />
      </header>
          ...
          <div key={a.email} className="group bg-white rounded-[3.5rem] p-10 border border-slate-100 shadow-xl relative overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-8 relative z-10">
              <div className="w-32 h-32 rounded-[2.8rem] overflow-hidden border-4 border-white shadow-2xl bg-slate-100 flex-shrink-0">
                <img src={a.image || `https://api.dicebear.com/7.x/bottts-neutral/svg?seed=${a.email}`} className="w-full h-full object-cover" />
              </div>
              <div className="grow space-y-4">
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleFollow(a.email)} className="px-6 py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest bg-slate-950 text-white hover:bg-teal-600 transition-all">{submitting === a.email ? "..." : "Suivre"}</button>
