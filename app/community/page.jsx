...
export default function CommunautePage() {
  ...
  async function loadAuthorsData() {
    try {
      // Récupération simultanée des profils et de la bibliothèque pour des stats réelles
      const [libRes, usersRes] = await Promise.all([
        fetch(`/api/github-db?type=library`),
        fetch(`/api/github-db?type=all_users`) 
      ]);
      ...
      // 1. Map des statistiques basées sur les publications (lectures, textes, certifs)
      const statsMap = publications.reduce((acc, pub) => {
        const email = pub.authorEmail?.toLowerCase().trim();
        if (!email) return acc;
        if (!acc[email]) {
          acc[email] = { certified: 0, likes: 0, views: 0, worksCount: 0 };
        }
      ...
      // 2. Fusion avec les profils réels pour avoir les noms de plume et photos à jour
      const consolidated = userProfiles.map(u => {
        const email = u.email?.toLowerCase().trim();
        const stats = statsMap[email] || { certified: 0, likes: 0, views: 0, worksCount: 0 };
        return {
          ...u,
          email,
      ...
      // 3. Tri par influence (Certifications > Lectures > Likes)
      const sortedAuthors = consolidated.sort((a, b) => 
        (b.certified * 100 + b.views) - (a.certified * 100 + a.views)
      );
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-20 bg-[#FCFBF9] min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        {authors.filter(a => a.name.toLowerCase().includes(searchTerm.toLowerCase())).slice(0, visibleCount).map((a) => (
          <div key={a.email} className="group bg-white rounded-[3.5rem] p-10 border border-slate-100 shadow-xl relative overflow-hidden transition-all hover:-translate-y-1">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-8 relative z-10">
              ...
              <div className="grow space-y-4">
                <h2 className="text-3xl font-black italic text-slate-900 tracking-tighter flex items-center gap-2">
                ...
                <div className="flex gap-2">
                  <button 
                    ...
                    className="px-6 py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest bg-slate-950 text-white hover:bg-teal-600 transition-all shadow-lg"
                  >
                    {submitting === a.email ? "..." : (a.followers?.includes(currentUser?.email) ? "Suivi" : "Suivre")}
                  </button>
                  <Link 
                    href={`/author/${encodeURIComponent(a.email)}`} 
                    className="px-6 py-3 bg-teal-600 text-white rounded-2xl text-[9px] font-black uppercase tracking-widest hover:bg-slate-900 transition-all flex items-center gap-2 shadow-lg"
                  >
