...
export default function CommunautePage() {
  ...
  async function loadAuthorsData() {
    try {
      // 1. Récupérer les stats de la bibliothèque
      const resLib = await fetch(`/api/github-db?type=library`); 
      const jsonLib = await resLib.json();
      
      // 2. Récupérer la base des utilisateurs pour synchroniser les photos de profil réelles
      ...
      const realUsersMap = Array.isArray(jsonUsers.content) 
        ? jsonUsers.content.reduce((acc, u) => {
            acc[u.email?.toLowerCase().trim()] = u.image || u.profilePic;
            return acc;
          }, {})
        : {};
        ...
        const statsMap = jsonLib.content.reduce((acc, pub) => {
          ...
          if (!acc[email]) {
            acc[email] = {
              ...
              email: email,
              // Priorité à l'image du compte réel, sinon celle de la publication, sinon null
    ...
    } catch (e) { toast.error("Le Cercle est inaccessible."); }
  ...
  const getBadges = (author) => {
    ...
    // BADGE ÉLITE (Record de lectures)
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-20 bg-[#FCFBF9] min-h-screen">
      ...
      <header className="flex flex-col lg:flex-row justify-between mb-24 gap-8">
        ...
        <input type="text" placeholder="Rechercher une plume..." className="w-full lg:w-96 bg-white border-2 border-slate-50 rounded-[2rem] px-8 py-5 shadow-xl outline-none focus:border-teal-500 transition-all" onChange={(e) => setSearchTerm(e.target.value)} />
      </header>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        {authors.filter(a => a.name.toLowerCase().includes(searchTerm.toLowerCase())).slice(0, visibleCount).map((a, index) => (
            ...
            <div className="flex flex-col sm:flex-row items-center gap-8 relative z-10">
              ...
              <div className="grow space-y-4 text-center sm:text-left">
                ...
                <div className="flex justify-center sm:justify-start gap-3">
                    <div className="bg-rose-50/50 px-3 py-1.5 rounded-xl text-center"><span className="block text-[8px] font-black text-rose-600 uppercase">Lectures</span><span className="text-sm font-black text-rose-700">{a.views}</span></div>
                    <div className="bg-teal-50/50 px-3 py-1.5 rounded-xl text-center"><span className="block text-[8px] font-black text-teal-600 uppercase">Textes</span><span className="text-sm font-black text-teal-700">{a.worksCount}</span></div>
                    <div className="bg-slate-50 px-3 py-1.5 rounded-xl text-center"><span className="block text-[8px] font-black text-slate-400 uppercase">Abonnés</span><span className="text-sm font-black text-slate-900">{a.followers?.length || 0}</span></div>
                </div>
                <div className="flex gap-2 justify-center sm:justify-start">
                  ...
                  <Link href={`/author/${encodeURIComponent(a.email)}`} className="px-6 py-3 bg-teal-600 text-white rounded-2xl text-[9px] font-black uppercase tracking-widest hover:bg-slate-900 transition-all flex items-center gap-2">Profil <ArrowRight size={14} /></Link>
                </div>
              </div>
            </div>
          </div>
