...
export const metadata = {
  ...
  description: "Politique de protection des données personnelles de la plateforme Lisible.",
};
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-10 pb-20 px-4">
      ...
      <header className="text-center space-y-4 pt-10">
        <div className="inline-flex p-4 bg-teal-50 dark:bg-teal-900/20 text-teal-600 dark:text-teal-400 rounded-[1.5rem] mb-2">
          <ShieldCheck size={40} />
        </div>
        <h1 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white tracking-tighter italic leading-tight">
          Politique de Confidentialité
        </h1>
        <p className="text-slate-400 dark:text-slate-500 font-medium italic text-sm">Dernière mise à jour : Janvier 2026</p>
      </header>

      <article className="bg-white dark:bg-slate-900 p-8 md:p-12 rounded-[3rem] prose prose-slate dark:prose-invert max-w-none border border-slate-50 dark:border-white/5 shadow-2xl shadow-slate-200/50 dark:shadow-none">
        <div className="space-y-10 text-slate-600 dark:text-slate-300 leading-relaxed">
          ...
          <section>
            <h2 className="flex items-center gap-3 text-xl font-black text-slate-900 dark:text-white mb-4 uppercase tracking-wider">
              <span className="text-teal-500 dark:text-teal-400"><Fingerprint size={24} /></span> 01. Collecte des données
            </h2>
            <p>
            ...
            <ul className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 list-none pl-0">
              <li className="flex items-start gap-3 bg-slate-50 dark:bg-white/5 p-4 rounded-2xl">
                <Database size={18} className="text-teal-600 dark:text-teal-400 shrink-0 mt-1" />
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">Identité : Nom, pseudonyme (nom de plume) et adresse email.</span>
              </li>
              <li className="flex items-start gap-3 bg-slate-50 dark:bg-white/5 p-4 rounded-2xl">
                <Eye size={18} className="text-teal-600 dark:text-teal-400 shrink-0 mt-1" />
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">Interactions : Textes publiés, commentaires, likes et abonnements.</span>
              </li>
            </ul>
          </section>
          ...
          <section>
            <h2 className="flex items-center gap-3 text-xl font-black text-slate-900 dark:text-white mb-4 uppercase tracking-wider">
              <span className="text-teal-500 dark:text-teal-400"><Lock size={24} /></span> 02. Utilisation & Sécurité
            </h2>
            <p>
          ...
          <section className="bg-slate-900 dark:bg-black p-8 rounded-[2.5rem] text-white border border-white/5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {["Droit d'accès", "Droit de rectification", "Suppression de compte", "Portabilité des textes"].map((droit, i) => (
                <div key={i} className="flex items-center gap-3 bg-white/5 border border-white/10 p-3 rounded-xl text-[10px] font-bold uppercase tracking-widest">
          ...
          <section>
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <a href="mailto:cmo.lablitteraire7@gmail.com" className="flex items-center gap-3 p-4 bg-teal-50 dark:bg-teal-900/20 text-teal-700 dark:text-teal-400 rounded-2xl font-bold text-sm flex-1 hover:bg-teal-100 dark:hover:bg-teal-900/30 transition-all active:scale-95 no-underline">
      ...
      <footer className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-6">
        <Link href="/" className="flex items-center gap-3 px-10 py-5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-black text-xs uppercase tracking-[0.2em] rounded-2xl hover:bg-teal-600 dark:hover:bg-teal-500 hover:text-white transition-all shadow-xl shadow-slate-200 dark:shadow-none active:scale-95">
          J'accepte ces conditions
        </Link>
        <Link href="/" className="flex items-center gap-2 px-8 py-4 bg-white dark:bg-slate-900 text-slate-400 dark:text-slate-500 font-black text-xs uppercase tracking-[0.2em] rounded-2xl border border-slate-100 dark:border-white/10 hover:text-teal-600 transition-all">
