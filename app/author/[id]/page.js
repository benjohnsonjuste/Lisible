...
import { 
  ArrowLeft, BookOpen, Loader2, UserPlus, UserMinus, Coins, 
  ChevronRight, HeartHandshake, ShieldCheck, Crown, Star, Share2 
} from "lucide-react";
...
export default function AuthorCataloguePage({ params }) {
  ...
  const fetchAuthorData = useCallback(async (id) => {
    ...
    try {
      ...
      // 1. Récupération du profil réel via l'API (data/users)
      const userRes = await fetch(`/api/github-db?type=user&id=${encodeURIComponent(authorEmail)}`);
      const userData = await userRes.json();
      if (!userData?.content) throw new Error("Auteur introuvable");
      setAuthor(userData.content);

      // 2. Récupération des publications et calcul des statistiques (data/publications)
      ...
      if (indexData?.content) {
        ...
        const viewsByAuthor = publications.reduce((acc, pub) => {
          const email = pub.authorEmail?.toLowerCase().trim();
          if (email) acc[email] = (acc[email] || 0) + Number(pub.views || 0);
          return acc;
        }, {});
        
        setGlobalMaxViews(Math.max(...Object.values(viewsByAuthor), 0));
        
        // Filtrage des œuvres de cet auteur
      ...
      toast.error("Profil introuvable dans le Cercle."); 
  ...
  const handleFollow = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success(isFollowing ? "Désabonné." : "Vous suivez maintenant cette plume !");
      }
    } catch (err) {
  ...
  const handleShare = async () => {
    const name = author?.penName || author?.name || "Une Plume Lisible";
    const shareData = { 
      title: `${name} sur Lisible`, 
      text: `Découvrez l'univers littéraire de ${name} ✨`, 
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-12 bg-[#FCFBF9] min-h-screen space-y-16">
      ...
      <header className="relative flex flex-col md:flex-row items-center gap-10 bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-xl">
        <button 
          onClick={() => router.back()} 
          className="absolute top-8 left-8 p-3 bg-slate-50 rounded-2xl hover:bg-slate-100 transition-colors"
        >
          <ArrowLeft size={20} />
        </button>
        ...
        <div className="text-center md:text-left grow space-y-4">
          <div className="space-y-1">
            <p className="text-[9px] font-black uppercase tracking-[0.4em] text-teal-600">Plume du Cercle</p>
          ...
          <div className="flex flex-wrap gap-2 justify-center md:justify-start">
            ...
            <div className="bg-teal-50 text-teal-600 px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest flex items-center gap-2">
              <Coins size={12}/> {author?.li || 0} Li
            </div>
            <div className="bg-slate-50 text-slate-400 px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest">
              {author?.followers?.length || 0} Abonnés
            </div>
          </div>

          {author?.bio && (
            <p className="text-slate-500 text-sm italic max-w-lg leading-relaxed line-clamp-2">
              "{author.bio}"
            </p>
          )}
        </div>
        ...
        <div className="flex flex-col gap-3 w-full md:w-auto">
          <button 
            ...
            disabled={submitting}
            className={`px-8 py-4 rounded-2xl flex items-center justify-center gap-2 font-black text-[10px] uppercase tracking-widest transition-all shadow-lg ${isFollowing ? "bg-slate-100 text-slate-400" : "bg-slate-900 text-white hover:bg-teal-600 shadow-slate-900/10"}`}
          >
          ...
          <div className="flex gap-2">
            <Link 
              ...
              className="grow bg-rose-600 text-white px-6 py-4 rounded-2xl flex items-center justify-center gap-2 font-black text-[10px] uppercase tracking-widest shadow-lg shadow-rose-600/20 hover:bg-rose-700 transition-all"
            >
              <HeartHandshake size={18} /> Soutenir
            </Link>
            <button 
              onClick={handleShare} 
              className="bg-white border-2 border-slate-100 px-5 py-4 rounded-2xl text-slate-400 hover:bg-slate-50 transition-all"
            >
              <Share2 size={18}/>
            </button>
          </div>
      ...
      <section className="space-y-8">
        <div className="flex items-center justify-between border-b border-slate-100 pb-4">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-slate-300">
            {texts.length} {texts.length > 1 ? 'publications' : 'publication'}
          </span>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {texts.map(txt => (
              <Link 
                href={`/texts/${txt.id}`} 
                key={txt.id} 
                className="group p-8 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:border-teal-100 transition-all relative overflow-hidden"
              >
                <div className="absolute top-0 left-0 w-2 h-full bg-slate-50 group-hover:bg-teal-500 transition-colors" />
                <p className="text-[8px] font-black uppercase tracking-widest text-teal-600 mb-2">
                  {txt.category || "Littérature"}
                </p>
                <h3 className="text-2xl font-black text-slate-900 group-hover:text-teal-600 transition-colors italic mb-6 leading-tight">
                  {txt.title}
                </h3>
                <div className="flex justify-between items-center pt-6 border-t border-slate-50">
                  <div className="flex items-center gap-4">
                    ...
                    {(txt.certified > 0) && (
                      <span className="bg-teal-50 text-teal-600 text-[8px] font-black px-2 py-1 rounded-lg uppercase">
                        Certifié
                      </span>
                    )}
                  </div>
