...
export default function AuthorCataloguePage({ params }) {
  ...
  const fetchAuthorData = useCallback(async (id) => {
    setLoading(true);
    try {
      const authorEmail = decodeURIComponent(id);
      ...
      if (indexData?.content) {
        ...
        setGlobalMaxViews(Math.max(...Object.values(viewsMap)));
        setTexts(indexData.content.filter(t => t.authorEmail?.toLowerCase() === authorEmail.toLowerCase()));
      }
    } catch (e) { toast.error("Auteur introuvable"); }
  ...
  const handleShare = async () => {
    const shareData = { title: author?.penName, text: `Découvrez les œuvres de ${author?.penName} sur Lisible ✨`, url: window.location.href };
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-12 bg-[#FCFBF9] min-h-screen space-y-16">
      <header className="relative flex flex-col md:flex-row items-center gap-10 bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-xl">
        <button onClick={() => router.back()} className="absolute top-8 left-8 p-3 bg-slate-50 rounded-2xl"><ArrowLeft size={20} /></button>
        <div className="w-40 h-40 rounded-[2.5rem] overflow-hidden border-4 border-white shadow-2xl relative">
          <img src={author?.image || `https://api.dicebear.com/7.x/adventurer-neutral/svg?seed=${author?.email}`} className="w-full h-full object-cover" />
        ...
        <div className="text-center md:text-left grow space-y-4">
          <h1 className="text-5xl font-black text-slate-900 tracking-tighter italic flex items-center gap-3 justify-center md:justify-start">
            {author?.penName || author?.name} {author?.certified > 0 && <ShieldCheck className="text-teal-500" size={28} />}
          </h1>
        ...
        <div className="flex flex-col gap-3 w-full md:w-auto">
          ...
          <button onClick={handleShare} className="bg-white border-2 border-slate-100 p-4 rounded-2xl flex items-center justify-center hover:bg-slate-50 transition-all"><Share2 size={18}/></button>
        </div>
      </header>
