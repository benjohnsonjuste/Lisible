...
export default function Auditorium() {
  ...
  return (
    <div className="max-w-5xl mx-auto py-12 px-4 min-h-screen">
      ...
      {!loading && (  
        <div className="mt-20 pt-12 border-t border-slate-100 w-full" />  
      )}  
    </div>
  );
}
...
function PodcastCard({ podcast, isActive, onPlay, onPause, setPodcasts }) {
  ...
  return (
    <div className={`group bg-white border p-6 rounded-[2.5rem] transition-all duration-300 ${isActive ? 'border-indigo-500 shadow-2xl scale-[1.02]' : 'border-slate-100 shadow-xl hover:shadow-2xl'}`}>
      <div className="flex items-start gap-5">
        <button
          onClick={togglePlay}
          className={`w-16 h-16 shrink-0 rounded-2xl flex items-center justify-center transition-all ${isActive ? 'bg-indigo-600 text-white animate-pulse' : 'bg-slate-900 text-white hover:bg-indigo-600'}`}
        >
