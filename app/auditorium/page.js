...
export default function Auditorium() {
  ...
  // Injection directe du script publicitaire Social Bar
  useEffect(() => {
    if (!loading && adRef.current && !adRef.current.hasChildNodes()) {
      const script = document.createElement("script");
      script.src =
        "https://pl28594689.profitablecpmratenetwork.com/62/bc/8f/62bc8f4d06d16b0f6d6297a4e94cfdfd.js";
      script.async = true;
  ...
  return (
    <div className="max-w-5xl mx-auto py-12 px-4 min-h-screen">
      <div className="mb-12 text-center">
        ...
        <p className="text-sm font-bold uppercase tracking-[0.2em] text-slate-400 mt-2">
          Archives des transmissions
        </p>
      </div>
        ...
        >
          <div className="flex items-center gap-4 ml-2">
            ...
            <span className="text-white font-bold tracking-tight">
              Accéder au studio pour lancer un podcast
            </span>
          </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-slate-50 rounded-[3rem] border-2 border-dashed border-slate-200">
          ...
          <p className="text-slate-500 font-medium">
            Aucun podcast n'a encore été diffusé.
          </p>
        </div>
      )}

      {/* Zone publicité Social Bar */}
      {!loading && (
...
function PodcastCard({ podcast, isActive, onPlay, onPause, setPodcasts }) {
  ...
  return (
    <div
      className={`group bg-white border p-6 rounded-[2.5rem] transition-all duration-300 ${
        isActive
