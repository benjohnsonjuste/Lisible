...
import {
  ArrowLeft, Trophy, Maximize2, Loader2, Sparkles, Megaphone, Ghost, Sun, Zap, AlertTriangle, Heart, Share2
} from "lucide-react";
...
export default function TextContent() {
  ...
  const loadContent = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      if (indexRes.ok) {
        ...
        setAllTexts((indexData.content || []).sort((a, b) => Number(b.certified || 0) - Number(a.certified || 0)));
      }
    } catch (e) {
  ...
  const handleLike = async () => {
    ...
    try {
        ...
        if (res.ok) toast.success("Coup de cœur enregistré !");
