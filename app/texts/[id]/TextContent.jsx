...
export default function TextContent() {
  ...
  // Utiliser la photo du compte si l'utilisateur connecté est l'auteur, sinon utiliser authorPic ou le fallback
  const finalAuthorPic = (user?.email === text.authorEmail && user?.pic) 
    ? user.pic 
    : (text.authorPic || `https://api.dicebear.com/7.x/adventurer-neutral/svg?seed=${text.authorEmail}`);
             ...
             <header className={`mb-16 space-y-8 transition-all duration-1000 ${isFocusMode ? 'opacity-40 blur-[1px]' : 'opacity-100'}`}>
                ...
                <div className="flex items-center gap-5 pt-8 border-t border-slate-100">
                   <div className="w-14 h-14 rounded-2xl bg-slate-900 border-2 border-white shadow-xl overflow-hidden">
                      <img src={finalAuthorPic} className="w-full h-full object-cover" alt="" />
                   </div>
                   <div>
