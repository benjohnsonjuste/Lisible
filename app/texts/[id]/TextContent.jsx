...
const TextContent = ({ id }) => {
  ...
  const mood = useMemo(() => {
    ...
    const sets = [
      {
        key: "melancholic",
        words: ["ombre", "triste", "nuit", "mort", "seul", "pleure", "vide", "silence"]
      },

      {
        key: "luminous",
        words: ["soleil", "joie", "amour", "clair", "vie", "rire", "espoir", "lumière"]
      },

      {
        key: "epic",
        words: ["force", "guerre", "feu", "épée", "sang", "combat", "fureur", "destin"]
      },

      {
        key: "soothing",
        words: ["calme", "paix", "vent", "doux", "plage", "repos", "harmonie", "nature"]
      }
    ];
  ...
  const handleLike = async () => {
    if (!user) {
      return toast.error("Connectez-vous pour aimer ce texte");
    }
  ...
  // --- CONTENU AVEC PUBLICITÉS INTÉGRÉES ---
        ...
        <div className="whitespace-pre-wrap">
          {paragraphs.map((p, i) => (
            <React.Fragment key={i}>
              ...
              {/* PUBLICITÉ APRÈS CHAQUE 4 PARAGRAPHES */}
              {(i + 1) % 4 === 0 && (
                <InTextAd />
              )}
            </React.Fragment>
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center min-h-screen font-serif animate-pulse">
        Immersion en cours...
      </div>
    );
  }
  ...
  ].includes(data.authorEmail);

  const isNovelDuel =
    data.genre === "Duel Des Nouvelles" ||
    data.category === "Duel Des Nouvelles";

  const isBattlePoetique =
    data.isConcours === true ||
    ["Battle Poétique", "Battle Poétique International"].includes(
  ...
  return (
    <div
      className={`min-h-screen transition-all duration-1000 ${
        isFocusMode ? "bg-[#050505]" : mood.bg
      }`}
    >
      <div className="fixed top-0 left-0 w-full h-1 z-[100] bg-black/10">
        <div
          className={`h-full transition-all duration-300 ${mood.accent.replace(
            "text",
            "bg"
          )}`}
      ...
      <nav
        className={`fixed top-0 w-full z-[90] transition-all p-6 flex justify-between items-center ${
          isFocusMode ? "opacity-0" : "opacity-100"
        }`}
      >
        <button
          onClick={() => router.back()}
          className="p-3 bg-white/90 backdrop-blur-md rounded-xl shadow-md border border-slate-200 text-slate-800 hover:scale-105 transition-transform"
        >
          <ArrowLeft size={20} />
        </button>
        ...
        <button
          ...
          className="p-3 bg-slate-900 text-white rounded-xl shadow-xl hover:bg-teal-700 transition-all flex items-center gap-2 text-sm font-bold"
        >
          <Maximize2 size={18} />
          Focus
        </button>
      ...
      {isFocusMode && (
        <button
          ...
          className="fixed top-8 right-8 z-[110] p-4 rounded-full bg-white/5 text-white/40 hover:text-white hover:bg-white/10 transition-all backdrop-blur-sm border border-white/10"
        >
          <Minimize2 size={24} />
        </button>
      )}
      ...
      <main className="max-w-3xl mx-auto px-6 pt-32 pb-48 relative">
        ...
        <header
          className={`mb-20 space-y-8 transition-all duration-700 ${
            isFocusMode
              ? "opacity-20 blur-md scale-95"
              : "opacity-100"
          }`}
        >
          <div className="flex flex-wrap items-center gap-3">
            ...
            {mood.icon && (
              <span
                className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest shadow-sm ${mood.ui}`}
              >
            ...
            <div className="ml-auto flex items-center gap-4 text-[11px] font-bold text-slate-400 uppercase tracking-tighter">
              ...
              <span className="flex items-center gap-1.5 bg-slate-100 px-3 py-1 rounded-md">
                <Clock size={14} />
                {Math.ceil((data.content?.length || 0) / 1000)} min
              </span>
            </div>
          </div>

          <h1
            className={`text-5xl sm:text-7xl font-serif font-black italic leading-[1.1] tracking-tight ${mood.title}`}
          >
            {data.title}
          </h1>
          ...
          <div className="flex items-center gap-5 pt-8 border-t border-slate-200/50">
            <div className="w-14 h-14 rounded-2xl bg-slate-200 border-2 border-white shadow-xl overflow-hidden shrink-0">
              <img
                src={
                  authorProfile ||
                  `https://api.dicebear.com/7.x/adventurer-neutral/svg?seed=${data.authorEmail}`
                }
            ...
            <div>
              <p
                className={`text-[10px] font-black uppercase tracking-[0.2em] mb-1 flex items-center gap-1.5 ${mood.accent}`}
              >
                <Sparkles size={12} />
                {isAnnouncementAccount
                  ? "Officiel"
              ...
              <p className={`text-xl font-bold italic ${mood.title}`}>
                {data.authorName}
              </p>
            </div>
          </div>
        ...
        <SecurityLock>
          <article
            className={`font-serif leading-[2] text-xl sm:text-[23px] transition-colors duration-500 ${
              isFocusMode ? "text-slate-200" : mood.text
            }`}
          >
            {renderedContent}
          </article>
        ...
        <section
          className={`mt-32 space-y-32 transition-all ${
            isFocusMode
      ...
      <FloatingActions
        ...
        onGift={() =>
          user
            ...
            : toast.error("Connectez-vous pour offrir des Li")
        }
        textId={id}
        isLoading={isLiking}
      />
