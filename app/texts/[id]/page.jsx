  ...
  const handleBookmark = async () => {
    ...
    try {
      const res = await fetch('/api/github-db', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ textId: id, userEmail: user.email, action: isBookmarked ? "remove" : "add" })
      });
      if (res.ok) {
        const newStatus = !isBookmarked;
        setIsBookmarked(newStatus);
        const bKey = `bookmarks_${user.email}`;
        let localB = JSON.parse(localStorage.getItem(bKey) || "[]");
        if (newStatus) localB.push(id);
        else localB = localB.filter(bid => bid !== id);
        localStorage.setItem(bKey, JSON.stringify(localB));
        toast.success(newStatus ? "Ajouté aux signets" : "Retiré des signets");
      }
          ...
          <div className="flex gap-2">  
            {!isStaffText && (  
              <>  
                ...
                <button onClick={handleBookmark} disabled={isBookmarking} className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border text-[10px] font-black transition-all ${isBookmarked ? 'bg-teal-600 border-teal-600 text-white' : 'bg-white dark:bg-slate-900'}`}>
                  {isBookmarking ? <Loader2 size={14} className="animate-spin" /> : <Bookmark size={14} className={isBookmarked ? "fill-white" : ""} />}
                </button>
              </>  
            )}  
        ...
        <article className="animate-in fade-in slide-in-from-bottom-8 duration-700">  
          ...
          <div className={`flex items-center gap-4 mb-16 transition-all duration-700 ${isFocusMode ? 'opacity-0' : ''}`}>  
            <div className="relative w-12 h-12 rounded-full overflow-hidden bg-teal-100 border border-teal-200">  
              <Image src={text.authorPic || `https://api.dicebear.com/7.x/initials/svg?seed=${text.authorName}`} alt={text.authorName} fill className="object-cover" />  
            </div>  
            <div>  
          ...
          {(text.image || text.imageBase64) && (  
            <div className="mb-12 rounded-[2rem] overflow-hidden shadow-2xl border border-slate-100 bg-white">  
              <img src={text.image || text.imageBase64} alt={text.title} className="w-full h-auto object-cover max-h-[500px]" />  
            </div>  
          )}  
          ...
          {/* Sceau de certification placé ici pour valider la lecture complète */}
          <div className="mb-24">
            <SceauCertification 
...
export async function getStaticProps({ params }) {
  try {
    ...
    return { props: { initialText, id: params.id, allTexts: recommendations }, revalidate: 60 };
