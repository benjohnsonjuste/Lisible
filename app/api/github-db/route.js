...
if(!isMatch)return NextResponse.json({error:"Mot de passe incorrect"},{status:401});const {password,...safeUser}=file.content;return NextResponse.json({success:true,user:safeUser});}
...
if(action==='restore_account'){const file=await getFile(targetPath);if(!file)return NextResponse.json({error:"Compte introuvable"},{status:404});file.content.status="active";delete file.content.deletedAt;file.content.notifications.unshift({id:`restore_${Date.now()}`,type:"security",message:"Bon retour ! Votre compte a été restauré avec succès.",date:new Date().toISOString(),read:false});await updateFile(targetPath,file.content,file.sha,`♻️ Restore Account: ${emailToUse}`);const {password,...safeUser}=file.content;return NextResponse.json({success:true,user:safeUser});}
