...
export default function AccountPage() {
  ...
  const handleDeleteAccount = async () => {
    ...
    if (!confirm) return;
    setIsDeleting(true);
    toast.loading("Suppression définitive en cours...");
    try {
    ...
    } catch (e) { toast.error("Erreur système"); setIsDeleting(false); }
  };
        ...
        <section className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white rounded-[3rem] p-8 md:p-12 shadow-xl border border-slate-50 space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <InputBlock label="Nouveau mot de passe" type="password" value={passData.new} onChange={v => setPassData({...passData, new: v})} />
            </div>
            <button disabled={isUpdatingPass} onClick={handleUpdatePassword} className="w-full py-5 border-2 border-slate-900 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-slate-900 hover:text-white transition-all">
              {isUpdatingPass ? <Loader2 className="animate-spin mx-auto" /> : "Actualiser mon mot de passe"}
            </button>
          </div>
