  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-10 space-y-12">
      <header className="space-y-4">
        <div className="inline-flex p-3 bg-slate-900 text-teal-400 rounded-2xl"><UsersIcon size={24} /></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {authors.map((a, index) => {
          ...
          return (
            <div key={index} className={`relative bg-white rounded-[3rem] p-8 border transition-all duration-500 shadow-xl ${isNumberOne ? 'ring-4 ring-amber-100 border-amber-200' : 'border-slate-100'}`}>
              {isNumberOne && (
              ...
              <div className="flex justify-between items-start">
                ...
                {!isMe && (
                  <button onClick={() => handleSubscription(a, isSubscribed)} className={`p-4 rounded-2xl transition-all ${isSubscribed ? 'bg-slate-100 text-slate-400' : 'bg-slate-900 text-white hover:bg-teal-600'}`}>
                    {isSubscribed ? <UserMinus size={20} /> : <UserPlus size={20} />}
                  </button>
                )}
              </div>
              <div className="mt-8 h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                <div className={`h-full transition-all duration-1000 ${subscribersCount >= 250 ? 'bg-amber-400' : 'bg-teal-500'}`} style={{ width: `${progress}%` }} />
              </div>
              <Link href={`/auteur/${encodeURIComponent(a.email)}`} className="mt-6 flex items-center justify-center gap-3 w-full py-4 bg-slate-50 rounded-2xl text-[10px] font-black text-slate-500 hover:bg-slate-900 hover:text-white transition-all uppercase tracking-widest">
