    ...
    <div className="max-w-5xl mx-auto px-6 py-12 space-y-16 animate-in fade-in duration-1000">
      <header className="relative flex flex-col md:flex-row items-center gap-10 bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-2xl">
        ...
        <div className="flex flex-col gap-3">
            <button onClick={() => {}} className={`px-8 py-4 rounded-2xl flex items-center gap-3 font-black uppercase text-[10px] tracking-widest shadow-sm border active:scale-95 ${isFollowing ? "bg-slate-100 text-slate-500 border-slate-200" : "bg-teal-600 text-white border-transparent shadow-md"}`}>
              {isFollowing ? <UserMinus size={16} /> : <UserPlus size={16} />} {isFollowing ? "Désabonner" : "Suivre la plume"}
            </button>
            <Link href={`/shop?for=${encodeURIComponent(author?.email)}`} className="bg-slate-900 text-white px-8 py-6 rounded-[2rem] flex flex-col items-center gap-2 hover:bg-teal-600 transition-all shadow-xl">
              <HeartHandshake /> <span className="text-[10px] font-black uppercase tracking-widest">Soutenir</span>
            </Link>
        </div>
      </header>

      {/* Galerie des textes... (Code existant inchangé) */}
