...
export default function AuthorCataloguePage({ initialAuthor = null, initialTexts = [] }) {
  ...
  const fetchAuthorData = useCallback(async (targetEmail) => {
    if (initialAuthor && initialAuthor.email === targetEmail) return;
    setLoading(true);
    try {
  ...
  if (loading && !author) return (
    <div className="flex h-screen flex-col items-center justify-center gap-4 font-sans">
      ...
      <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Appel de la Plume...</p>
    </div>
  );
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-12 space-y-16 animate-in fade-in duration-1000 font-sans">
      <header className="relative flex flex-col md:flex-row items-center gap-10 bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-2xl">
        ...
        <div className="w-40 h-40 rounded-[2.5rem] bg-slate-900 border-8 border-white shadow-2xl overflow-hidden shrink-0">
          <img src={author?.profilePic || `https://api.dicebear.com/7.x/shapes/svg?seed=${author?.email}`} className="w-full h-full object-cover" alt="Profile" />
        </div>
        <div className="text-center md:text-left grow space-y-4">
          ...
          <div className="flex flex-wrap justify-center md:justify-start gap-3">
            ...
            <div className="bg-teal-50 text-teal-600 px-4 py-2 rounded-xl flex items-center gap-2 border border-teal-100">
               <ShieldCheck size={14} /> <span className="text-[10px] font-black uppercase">{texts.reduce((acc, curr) => acc + (Number(curr.totalCertified) || 0), 0)} Li Gagnés</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6">
          {texts.map((txt) => (
            <Link href={`/texts/${txt.id}`} key={txt.id} className="group flex flex-col md:flex-row items-center justify-between p-8 bg-white rounded-[2.5rem] border border-slate-50 hover:border-teal-500/30 transition-all shadow-sm hover:shadow-xl">
              <div className="space-y-3">
                <span className="text-[8px] font-black bg-slate-900 text-white px-2 py-0.5 rounded uppercase">{txt.isConcours ? "Battle" : (txt.category || "Œuvre")}</span>
                <h3 className="text-3xl font-black text-slate-900 group-hover:text-teal-600 transition-colors italic tracking-tight leading-tight">{txt.title}</h3>
              </div>
          ...
          {texts.length === 0 && (
            <div className="text-center py-20 bg-slate-50 rounded-[3rem] border-2 border-dashed border-slate-200">
               <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Cette plume n'a pas encore laissé de trace.</p>
            </div>
          )}
        </div>
      </div>
...
// Logique de pré-chargement pour rendre la page instantanée (SSR)
export async function getServerSideProps(context) {
  ...
  try {
    ...
    const users = await Promise.all(promises);
    const author = users.find(u => u.email?.toLowerCase() === cleanEmail) || null;

    // Récupération de ses textes
    const textsRes = await fetch(`https://api.github.com/repos/benjohnsonjuste/Lisible/contents/data/publications?t=${t}`);
    const tFiles = await textsRes.json();
    const tPromises = tFiles.filter(f => f.name.endsWith('.json')).map(f => fetch(f.download_url).then(r => r.json()));
    const allTexts = await Promise.all(tPromises);
    const authorTexts = allTexts.filter(t => t.authorEmail?.toLowerCase() === cleanEmail).sort((a, b) => new Date(b.date) - new Date(a.date));

    return {
      props: {
        initialAuthor: author,
        initialTexts: authorTexts
      }
    };
  } catch (e) {
    return { props: { initialAuthor: null, initialTexts: [] } };
  }
}
