...
import { InTextAd } from "@/components/InTextAd"; // Importation du nouveau composant
...
export default function TextPage() {
  ...
  const handlePostComment = async () => {
    ...
    try {
      const res = await fetch("/api/texts", {
        method: "PATCH",
        headers: {"Content-Type":"application/json"},
        body: JSON.stringify({ id: textId, action: "comment", payload: { userName: user.penName || user.name, text: newComment } })
      });
      if (res.ok) {
        const updated = await res.json();
        setText(updated);
  ...
  if (loading) return <div className="flex flex-col items-center justify-center py-40 gap-4"><Loader2 className="animate-spin text-teal-600" size={40} /></div>;

  if (!text) return <div className="py-40 text-center font-black uppercase text-[10px] text-slate-400">Texte introuvable</div>;

  // --- LOGIQUE D'INSERTION DE PUB ---
  // On divise le texte par les sauts de ligne pour gérer les paragraphes
    ...
    <div className="max-w-3xl mx-auto px-4 pb-20 space-y-12 animate-in fade-in duration-700">
      ...
      <article className="bg-white rounded-[3.5rem] p-8 md:p-16 shadow-2xl border border-slate-50 relative">
        <div className="mb-10 flex justify-between items-start">
          <h1 className="text-5xl md:text-7xl font-black italic tracking-tighter leading-[0.9] text-slate-900">{text.title}</h1>
          <button onClick={() => { navigator.clipboard.writeText(window.location.href); toast.success("Lien copié !"); }} className="p-4 bg-slate-50 rounded-2xl text-slate-400 hover:text-teal-500 active:scale-90"><Share2 size={24}/></button>
        </div>

        {/* AFFICHAGE DU CONTENU AVEC PUB INJECTÉE */}
        <div className="text-slate-800 leading-[1.8] font-serif text-xl md:text-2xl mb-16">
          {paragraphs.map((para, index) => (
            <React.Fragment key={index}>
              ...
              {/* Insertion de la pub après le 2ème paragraphe */}
              {index === 1 && <InTextAd />}
              
              {/* Si le texte est long (plus de 10 paragraphes), on en remet une après le 7ème */}
              {index === 6 && paragraphs.length > 10 && <InTextAd />}
            </React.Fragment>
          ))}
        </div>
        ...
        <div className="pt-8 border-t border-slate-100 flex items-center justify-between text-slate-400">
          <div className="flex gap-8">
            ...
            <button onClick={handleLike} className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all active:scale-125 ${text.likes?.includes(user?.email) ? 'bg-rose-50 text-rose-500' : 'bg-slate-50'}`}>
      ...
      {/* SECTION COMMENTAIRES... (Identique au reste du code) */}
        ...
        <div className="bg-white p-6 md:p-10 rounded-[2.5rem] border border-slate-100 shadow-xl">
          {user ? (
            <div className="space-y-4">
              <textarea value={newComment} onChange={(e)=>setNewComment(e.target.value)} className="w-full bg-slate-50 rounded-3xl p-6 min-h-[140px] outline-none font-medium" placeholder="Votre ressenti..." />
              <div className="flex justify-end"><button onClick={handlePostComment} disabled={isSubmitting || !newComment.trim()} className="bg-slate-900 text-white px-10 py-5 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] flex items-center gap-3 hover:bg-teal-600 transition-all">{isSubmitting ? <Loader2 size={16} className="animate-spin"/> : <Send size={16}/>} Publier</button></div>
            </div>
          ) : (
            <div className="text-center py-6"><Link href="/login" className="text-teal-600 font-black text-[10px] uppercase border-b-2 border-teal-600 pb-1">S'identifier pour commenter</Link></div>
          )}
        </div>
      </section>
    </div>
