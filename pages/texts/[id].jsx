...
export default function TextPage({ params }) {
  ...
  const handleComment = async () => {
    ...
    const newComment = { id: Date.now(), authorName: user.penName || user.name, message: comment.trim(), createdAt: new Date().toISOString() };
    try {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 pb-20 space-y-8 animate-in fade-in duration-700">
      ...
      <article className="bg-white rounded-[2.5rem] border border-slate-100 shadow-2xl">
        <div className="p-8 md:p-12 space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-slate-900 rounded-xl flex items-center justify-center text-teal-400 font-black uppercase shadow-lg">{text.authorName?.charAt(0)}</div>
      ...
      <section className="space-y-6">
        ...
        {user ? (
          <div className="bg-white p-6 rounded-[2rem] border border-slate-100 space-y-4 shadow-sm">
            <textarea value={comment} onChange={e => setComment(e.target.value)} placeholder="Votre avis..." className="w-full bg-slate-50 rounded-2xl p-4 border-none outline-none text-slate-800 min-h-[100px] resize-none" />
            <div className="flex justify-end">
              <button onClick={handleComment} disabled={isSubmitting || !comment.trim()} className="bg-slate-900 text-white py-3 px-8 rounded-xl text-[10px] font-black uppercase flex items-center gap-2">
