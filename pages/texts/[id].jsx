...
export default function TextPage() {
  ...
  const fetchPageData = useCallback(async () => {
    ...
    try {
      ...
      // 2. Fetch des textes avec anti-cache
      const resText = await fetch(`https://raw.githubusercontent.com/benjohnsonjuste/Lisible/main/data/textes.json?t=${Date.now()}`);
      const dataText = await resText.json();
      
      // LOGIQUE DE RECHERCHE ULTRA-ROBUSTE
      // On prend ce qui est avant le premier tiret : "1769461210822-hi" -> "1769461210822"
      const cleanIdFromUrl = String(rawId).split('-')[0].trim();
      
      const found = dataText.find(t => {
          const jsonId = String(t.id).trim();
          return jsonId === cleanIdFromUrl || jsonId === String(rawId).trim();
      });
      
      if (found) {
        setText(found);
        
        // 3. Charger les commentaires
        ...
        console.error("Aucune correspondance trouvée pour l'ID:", cleanIdFromUrl);
      }
    } catch (e) {
  ...
  if (!text) return (
    <div className="py-40 text-center px-6">
      ...
      <Link href="/bibliotheque" className="bg-slate-900 text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase">Retourner à la bibliothèque</Link>
    </div>
  );
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 pb-20 space-y-12 animate-in fade-in duration-700">
      ...
      <header className="pt-8 flex justify-between items-center">
        <Link href="/bibliotheque" className="text-slate-400 hover:text-teal-600 font-black text-[10px] uppercase flex items-center gap-2">
          <ArrowLeft size={16} /> Bibliothèque
        </Link>
        {isOwner && <button className="bg-slate-900 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase">Modifier</button>}
      </header>
      ...
      <article className="bg-white rounded-[2.5rem] border border-slate-100 shadow-2xl overflow-hidden p-8 md:p-14">
        <div className="flex items-center justify-between border-b border-slate-50 pb-8 mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-base font-black text-slate-900 italic">{text.authorName}</p>
            </div>
          </div>
        ...
        <h1 className="text-4xl md:text-6xl font-black text-slate-900 tracking-tighter italic leading-tight mb-8">{text.title}</h1>
        ...
        <footer className="mt-12 pt-8 border-t border-slate-50 flex justify-between">
          <button className="flex items-center gap-2 text-slate-400 hover:text-rose-500 transition-colors">
            <Heart size={20} /> <span className="text-xs font-bold uppercase">Aimer</span>
          </button>
          <button onClick={() => { navigator.clipboard.writeText(window.location.href); toast.success("Lien copié"); }} className="flex items-center gap-2 text-slate-400 hover:text-teal-500">
            <Share2 size={20} /> <span className="text-xs font-bold uppercase">Partager</span>
          </button>
        </footer>
      </article>
        ...
        {user ? (
          <div className="bg-white p-6 rounded-[2rem] shadow-xl border border-slate-50 space-y-4">
            ...
            <div className="flex justify-end">
              <button onClick={handlePostComment} disabled={!newComment.trim() || isSubmitting} className="bg-slate-900 text-white px-8 py-3 rounded-xl text-[10px] font-black uppercase flex items-center gap-2">
          ...
          <div className="p-8 text-center bg-slate-100 rounded-[2rem]">
            <Link href="/login" className="text-[10px] font-black text-teal-600 uppercase">Se connecter pour commenter</Link>
          </div>
        )}
        <div className="space-y-4">
          {comments.map((c, i) => (
            <div key={i} className="bg-white p-6 rounded-[2rem] border border-slate-100 flex gap-4">
              <div className="w-10 h-10 shrink-0 bg-slate-900 rounded-xl flex items-center justify-center text-teal-400 font-black">{c.userName?.charAt(0)}</div>
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-sm font-black">{c.userName}</span>
                  <span className="text-[8px] text-slate-300 uppercase">{new Date(c.date).toLocaleDateString()}</span>
                </div>
