...
export default function TextPublishing() {
  ...
  const handlePublish = async (e) => {
    ...
    if (authError || !user) return alert("⚠️ Connectez-vous pour publier.");
    ...
    try {
      ...
      if (coverFile && !coverUrl) return alert("❌ Échec de l’upload de l’image.");
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex justify-center items-start py-10">
      <form onSubmit={handlePublish} className="bg-white shadow-md rounded-2xl p-8 w-full max-w-3xl space-y-6">
        <h1 className="text-2xl font-bold text-center text-primary">✍️ Publier un texte sur Lisible</h1>
        ...
        <div>
          ...
          <textarea required value={textData.content} onChange={e => handleChange("content", e.target.value)} placeholder="Écrivez votre texte ici..." className="w-full min-h-[220px] border border-input rounded-md px-3 py-2 text-sm" />
