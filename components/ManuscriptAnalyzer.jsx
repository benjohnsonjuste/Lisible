...
export default function ManuscriptAnalyzer() {
  ...
  const steps = [
    ...
    "Exécution des protocoles de lecture bêta neuronale...",
    "Génération de l'index d'acceptation et des corrections...",
  ...
  // Actions d'édition interactive dans le texte source
  const acceptProofreadSuggestion = (suggestion) => {
    // Remplacement de la première occurrence trouvée de l'expression d'origine
    const updatedText = text.replace(suggestion.original, suggestion.corrected.split(' / ')[0]);
    setText(updatedText);
    
    // Nettoyage de l'interface en retirant la suggestion traitée
    ...
    <div className="bg-slate-950 text-slate-100 p-6 md:p-12 font-sans rounded-2xl border border-slate-900 max-w-5xl mx-auto space-y-8">
      <header className="border-b border-slate-800 pb-6">
        <h1 className="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-emerald-400 to-cyan-400 bg-clip-text text-transparent flex items-center gap-2">
          PlumLocal <span className="text-xs font-mono px-2 py-1 bg-slate-800 text-slate-400 rounded-full">v2.5 (Édition & Bêta-lecture)</span>
        </h1>
      </header>
