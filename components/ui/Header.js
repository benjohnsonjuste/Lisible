...
import Button from "./Button"; // importe default (compatible)
import AppIcon from "../AppIcon"; // logo / icônes (note le chemin)
import InstallPrompt from "./InstallPrompt";
import { HelpCircle, User, BarChart3, BookOpen, LogOut, Plus } from "lucide-react";
...
const Header = () => {
  ...
  const handleLogout = () => {
    // Implémenter la logique d'auth si besoin
    router.push("/login");
  };
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-background border-b border-border shadow-sm">
      <div className="flex justify-between items-center h-16 px-4">
        ...
        <div className="flex items-center space-x-2">
          <Button variant="ghost" className="p-2" onClick={() => setIsMoreMenuOpen(!isMoreMenuOpen)}>
