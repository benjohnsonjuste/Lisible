...
export default function Comment({ 
  textId, 
  comments = [], 
  user,           // L'utilisateur connecté (celui qui commente)
  authorEmail,    // IMPORTANT: L'email de l'auteur à notifier
  textTitle,      // Le titre pour la notification
  onCommented 
}) {
    ...
    try {
      ...
      const res = await fetch('/api/github-db', {
        ...
        body: JSON.stringify({ 
          ...
          userName: user.penName || user.name || "Une plume", 
          userPic: user.image || null,
          comment: msg.trim(), 
          authorEmail: authorEmail, // Transmis pour modifier data/users/
          textTitle: textTitle,      // Pour le message de notification
      ...
      if (res.ok) { 
        ...
        toast.success("Votre pensée a rejoint le Grand Livre", { id: tid }); 
        
        // Revalidation pour mettre à jour les pages statiques si nécessaire
        try {
  ...
  return (
    <div className="mt-32 max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-8 duration-1000 px-4">
      ...
      <div className="sticky bottom-8 z-50">
        <div className="bg-white/90 backdrop-blur-2xl p-3 rounded-[2.5rem] border border-white shadow-2xl flex items-center gap-3 ring-1 ring-slate-900/5 focus-within:ring-teal-500/20 transition-all">
          <input 
            value={msg} 
            ...
            placeholder={user ? "Partager une résonance..." : "Connectez-vous pour commenter"}
